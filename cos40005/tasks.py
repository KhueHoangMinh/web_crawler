...
def crawl_domain():
    ...
    url_con_type = domain.url_container_elements_type
    url_con_prop = domain.url_container_elements_property
    print(url_con_prop)
    try:
        cache = Cache.objects.get(url=domain.start_url)
        cache.domain = domain
        cache.status = False
        cache.visited = False
        cache.save()
    except Cache.DoesNotExist:
        Cache.objects.create(domain=domain, url=domain.start_url, status=False, visited=False)

    not_visited = Cache.objects.filter(visited=False)[:20]
    if not_visited and len(not_visited) > 0:
        ...
        for cache in not_visited:
            ...
            try:
                ...
                time.sleep(2)
                links = []
                if url_con_type is not None and len(url_con_prop) > 0:
                    for url_prop in url_con_prop:
                        anchor_tags = driver.find_element(url_con_type, url_prop).find_elements(By.TAG_NAME, 'a')
...
def crawl_property(domain_names):
    ...
    for domain_name in domain_names:
        ...
        images_type = domain.images_type
        images_property = domain.images_property

        brand_type = domain.brand_type
        brand_property = domain.brand_property

        price_type = domain.price_type
        price_property = domain.price_property

        type_type = domain.type_type
        type_property = domain.type_property

        transmission_type = domain.transmission_type
        transmission_property = domain.transmission_property

        engine_type = domain.engine_type
        engine_property = domain.engine_property

        seats_type = domain.seats_type
        seats_property = domain.seats_property

        gearbox_type = domain.gearbox_type
        gearbox_property = domain.gearbox_property

        date_type = domain.date_type
        date_property = domain.date_property

        odo_type = domain.odo_type
        odo_property = domain.odo_property
            ...
            for cache in caches:
                ...
                if title_type and title_property:
                    try:
                        ...
                        pass
                if images_type and images_property:
                    try:
                        images = [el.get_attribute("src") for el in driver.find_element(images_type, images_property).find_elements(By.TAG_NAME, "img")]
                    except Exception as e:
                        print(cache.url,e)
                if brand_type and brand_property:
                    try:
                        brand = driver.find_element(brand_type, brand_property).text
                    except:
                        pass
                if price_type and price_property:
                    try:
                        ...
                        pass
                if type_type and type_property:
                    try:
                        car_type = driver.find_element(type_type, type_property).text
                    except:
                        pass
                if transmission_type and transmission_property:
                    try:
                        transmission = driver.find_element(transmission_type, transmission_property).text
                    except:
                        pass
                if engine_type and engine_property:
                    try:
                        engine = driver.find_element(engine_type, engine_property).text
                    except:
                        pass
                if seats_type and seats_property:
                    try:
                        seats = driver.find_element(seats_type, seats_property).text
                    except:
                        pass
                if gearbox_type and gearbox_property:
                    try:
                        gearbox = driver.find_element(gearbox_type, gearbox_property).text
                    except:
                        pass
                if date_type and date_property:
                    try:
                        date = driver.find_element(date_type, date_property).text
                    except:
                        pass
                if odo_type and odo_property:
                    try:
                        odo = driver.find_element(odo_type, odo_property).text
                    except:
                        pass
                ...
                try:
                    if title is not None and price is not None:
                        property = Property(domain=domain, cache=cache, title=title, images=images, brand=brand, price=price, type=car_type, transmission=transmission,
                                            engine=engine, seats=seats, gearbox=gearbox, date=date, odo=odo, contact=contact,
